...
static void showBanner_logo(LogoMode mode) {
  ...
  case UTF8: {
    ...
    PrintAndLogEx(
        NORMAL,
        "  " _BLUE_("%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s"), sq,
        sq, sq, sq, sq, sq, tr, __, sq, sq, sq, tr, __, __, __, sq, sq, sq, tr,
        sq, sq, sq, sq, sq, tr, __);
    PrintAndLogEx(
        NORMAL,
        "  " _BLUE_("%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s"), sq,
        sq, tl, hl, hl, sq, sq, tr, sq, sq, sq, sq, tr, __, sq, sq, sq, sq, vl,
        bl, hl, hl, hl, sq, sq, tr);
    PrintAndLogEx(
        NORMAL,
        "  " _BLUE_("%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s"), sq,
        sq, sq, sq, sq, sq, tl, br, sq, sq, tl, sq, sq, sq, sq, tl, sq, sq, vl,
        __, sq, sq, sq, sq, tl, br);
    PrintAndLogEx(
        NORMAL,
        "  " _BLUE_("%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s"), sq,
        sq, tl, hl, hl, hl, br, __, sq, sq, vl, bl, sq, sq, tl, br, sq, sq, vl,
        __, bl, hl, hl, sq, sq, tr);
    PrintAndLogEx(NORMAL,
                  "  " _BLUE_("%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%"
                              "s%s%s") " " BANNERMSG1,
                  sq, sq, vl, __, __, __, __, __, sq, sq, vl, __, bl, hl, br,
                  __, sq, sq, vl, sq, sq, sq, sq, sq, tl, br);
    PrintAndLogEx(NORMAL,
                  "  " _BLUE_("%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%"
                              "s%s%s") " " BANNERMSG2,
                  bl, hl, br, __, __, __, __, __, bl, hl, br, __, __, __, __,
                  __, bl, hl, br, bl, hl, hl, hl, hl, br, __);
    break;
  }
  case ANSI: {
...
static void showBanner(void) {
  ...
  //    PrintAndLogEx(NORMAL, "
  //    43mNJLpgBVaTvyZmX9ajcohpvVkaRy1kbZPm8tqAb7itZgfuYecgkRF36rXrKFUkwEGeZedPsASRxgv4HPBHvJwyJdyvQuP");
  PrintAndLogEx(NORMAL, "");
  fflush(stdout);
...
static void prompt_compose(char *buf, size_t buflen, const char *promptctx,
                           const char *promptdev, const char *promptnet,
                           bool no_newline) {
  if (no_newline) {
    snprintf(buf, buflen - 1, PROXPROMPT_COMPOSE, promptdev, promptnet,
             promptctx);
  } else {
    snprintf(buf, buflen - 1, "\33[2K\r" PROXPROMPT_COMPOSE, promptdev,
             promptnet, promptctx);
  }
}
...
static int check_comm(void) {
  // If communications thread goes down. Device disconnected then this should
  // hook up PM3 again.
  if (IsCommunicationThreadDead() && g_session.pm3_present) {

#ifndef HAVE_READLINE
    PrintAndLogEx(INFO, _YELLOW_("OFFLINE") " mode. Use "_YELLOW_(
                            "\"hw connect\"") " to reconnect\n");
#endif
    prompt_dev = PROXPROMPT_DEV_OFFLINE;
    ...
    memcpy_filter_ansi(prompt_filtered, prompt, sizeof(prompt_filtered),
...
static int js_getchar(void) { return uart_read_stdin(); }
...
void
...
#endif
#endif
    main_loop(const char *script_cmds_file, char *script_cmd,
  ...
  if (script_cmds_file) {
    ...
    char *path;
    int res =
        searchFile(&path, CMD_SCRIPTS_SUBDIR, script_cmds_file, ".cmd", false);
    if (res == PM3_SUCCESS) {
  ...
  if (g_session.incognito) {
    ...
    if (searchHomeFilePath(&g_session.history_path, NULL, PROXHISTORY, true) !=
        PM3_SUCCESS) {
      g_session.history_path = NULL;
  ...
  check_script:
    ...
    if (current_cmdscriptfile()) {
      ...
      if (execCommand) {
        prompt_ctx =
            stdinOnPipe ? PROXPROMPT_CTX_STDIN : PROXPROMPT_CTX_SCRIPTCMD;
...
#if defined(_WIN32)
          // Check if color support needs to be enabled again in case the window
          // buffer did change
    ...
    if (cmd) {
      ...
      if (cmd[0] != '\0') {
        ...
        char prompt_filtered[PROXPROMPT_MAX_SIZE] = {0};
        memcpy_filter_rlmarkers(prompt_filtered, prompt,
...
static void dumpAllHelp(int markdown, bool full_help) {
  ...
  PrintAndLogEx(NORMAL, "\n%sProxmark3 command dump%s\n\n",
                ...
                markdown ? "" : "\n======================");
  PrintAndLogEx(NORMAL,
                "Some commands are available only if a Proxmark3 is actually "
                "connected.%s\n",
                markdown ? "  " : "");
...
const char *get_my_executable_path(void) { return my_executable_path; }
...
static void set_my_executable_path(void) {
  ...
  if (wai_getExecutablePath(my_executable_path, path_length, &dirname_length) !=
      -1) {
    my_executable_path[path_length] = '\0';
    my_executable_directory =
        (char *)calloc(dirname_length + 2, sizeof(uint8_t));
...
const char *get_my_user_directory(void) { return my_user_directory; }
...
static void show_help(bool showFullHelp, char *exec_name) {
...
#ifdef HAVE_PYTHON
  PrintAndLogEx(NORMAL,
                "        %s [[-p] <port>] [-b] [-w] [-f] [-c <command>]|[-l "
                "<lua_script_file>]|[-y <python_script_file>]|[-s "
                "<cmd_script_file>] [-i] [-d <0|1|2>]",
                exec_name);
#else  // HAVE_PYTHON
  PrintAndLogEx(NORMAL,
                "        %s [[-p] <port>] [-b] [-w] [-f] [-c <command>]|[-l "
                "<lua_script_file>]|[-s <cmd_script_file>] [-i] [-d <0|1|2>]",
                exec_name);
#endif // HAVE_PYTHON
  PrintAndLogEx(NORMAL,
                "        %s [-p] <port> --flash [--unlock-bootloader] [--image "
                "<imagefile>]+ [-w] [-f] [-d <0|1|2>]",
                exec_name);
  ...
  if (showFullHelp) {
    ...
    PrintAndLogEx(NORMAL, "      -w/--wait                           20sec "
                          "waiting the serial port to appear in the OS");
    PrintAndLogEx(NORMAL, "      -f/--flush                          output "
                          "will be flushed after every print");
    PrintAndLogEx(NORMAL,
                  "      -d/--debug <0|1|2>                  set debugmode");
    PrintAndLogEx(NORMAL, "\nOptions in client mode:");
    PrintAndLogEx(NORMAL, "      -t/--text                           dump all "
                          "interactive command list at once");
    PrintAndLogEx(NORMAL, "      --fulltext                          dump all "
                          "interactive command's help at once");
    PrintAndLogEx(NORMAL,
                  "      -m/--markdown                       dump all "
                  "interactive command list at once in markdown syntax");
    PrintAndLogEx(NORMAL,
                  "      -b/--baud                           serial port speed "
                  "(only needed for physical UART, not for USB-CDC or BT)");
    PrintAndLogEx(NORMAL,
                  "      -c/--command <command>              execute one "
                  "Proxmark3 command (or several separated by ';').");
    PrintAndLogEx(
        NORMAL,
...
#endif // HAVE_PYTHON
    PrintAndLogEx(NORMAL, "      -s/--script-file <cmd_script_file>  script "
                          "file with one Proxmark3 command per line");
    PrintAndLogEx(NORMAL,
                  "      -i/--interactive                    enter interactive "
                  "mode after executing the script or the command");
    PrintAndLogEx(NORMAL, "      --incognito                         do not "
                          "use history, prefs file nor log files");
    PrintAndLogEx(NORMAL, "      --ncpu <num_cores>                  override "
                          "number of CPU cores");
    PrintAndLogEx(NORMAL, "\nOptions in flasher mode:");
    PrintAndLogEx(NORMAL, "      --flash                             flash "
                          "Proxmark3, requires at least one --image");
    PrintAndLogEx(NORMAL, "      --reboot-to-bootloader              reboot "
                          "Proxmark3 into bootloader mode");
    PrintAndLogEx(NORMAL,
                  "      --unlock-bootloader                 Enable flashing "
                  "of bootloader area *DANGEROUS* (need --flash)");
    PrintAndLogEx(NORMAL,
                  "      --force                             Enable flashing "
                  "even if firmware seems to not match client version");
    PrintAndLogEx(NORMAL, "      --image <imagefile>                 image to "
                          "flash. Can be specified several times.");
    PrintAndLogEx(NORMAL, "\nOptions in memory dump mode:");
    PrintAndLogEx(NORMAL, "      --dumpmem <dumpfile>                dumps "
                          "Proxmark3 flash memory to file");
    PrintAndLogEx(NORMAL, "      --dumpaddr <address>                starting "
                          "address for dump, default 0");
    PrintAndLogEx(NORMAL, "      --dumplen <length>                  number of "
                          "bytes to dump, default 512KB");
    PrintAndLogEx(NORMAL, "      --dumpraw                           raw "
                          "address mode: dump from anywhere, not just flash");
    PrintAndLogEx(NORMAL, "\nExamples:");
    ...
    PrintAndLogEx(NORMAL,
                  "      %s " SERIAL_PORT_EXAMPLE_H
                  " -f                    -- flush output every time",
                  exec_name);
    PrintAndLogEx(NORMAL,
                  "      %s " SERIAL_PORT_EXAMPLE_H
                  " -w                    -- wait for serial port",
                  exec_name);
    PrintAndLogEx(NORMAL,
                  "      %s                                    -- runs the pm3 "
    ...
    PrintAndLogEx(NORMAL,
                  "      %s " SERIAL_PORT_EXAMPLE_H
                  " -c \"hf mf chk --1k\"   -- execute cmd and quit client",
                  exec_name);
    PrintAndLogEx(NORMAL,
                  "      %s " SERIAL_PORT_EXAMPLE_H
                  " -l hf_read            -- execute Lua script `hf_read` and "
                  "quit client",
                  exec_name);
    PrintAndLogEx(NORMAL,
                  "      %s " SERIAL_PORT_EXAMPLE_H
                  " -s mycmds.txt         -- execute each pm3 cmd in file and "
                  "quit client",
                  exec_name);
    PrintAndLogEx(NORMAL, "\n  to flash fullimage and bootloader:\n");
    PrintAndLogEx(NORMAL,
                  "      %s " SERIAL_PORT_EXAMPLE_H
                  " --flash --unlock-bootloader --image bootrom.elf --image "
                  "fullimage.elf",
                  exec_name);
#ifdef __linux__
    PrintAndLogEx(NORMAL, "\nNote (Linux):\nif the flasher gets stuck in "
                          "'Waiting for Proxmark3 to reappear on <DEVICE>',");
    PrintAndLogEx(NORMAL, "you need to blacklist Proxmark3 for modem-manager - "
                          "see documentation for more details:");
    PrintAndLogEx(
        NORMAL,
        "* "
        "https://github.com/RfidResearchGroup/proxmark3/blob/master/doc/md/"
        "Installation_Instructions/ModemManager-Must-Be-Discarded.md");
    PrintAndLogEx(
        NORMAL,
        "\nMore info on flashing procedure from the official Proxmark3 wiki:");
    PrintAndLogEx(
        NORMAL, "* https://github.com/Proxmark/proxmark3/wiki/Gentoo%%20Linux");
    PrintAndLogEx(
        NORMAL, "* https://github.com/Proxmark/proxmark3/wiki/Ubuntu%%20Linux");
    PrintAndLogEx(NORMAL, "* https://github.com/Proxmark/proxmark3/wiki/OSX\n");
#endif
  }
}

static int dumpmem_to_file(const char *filename, uint32_t addr, uint32_t len,
                           bool raw, bool in_bootloader) {
  ...
  if (GetFromDevice(type, buffer, len, addr, NULL, 0, NULL, 1000, true)) {
    res = PM3_SUCCESS;
    readlen = len; // GetFromDevice does not report the actual number of bytes
                   // received.
  }
...
static int dumpmem_pm3(char *serial_port_name, const char *filename,
                       uint32_t addr, uint32_t len, bool raw) {
  int ret = PM3_EUNDEF;
  ...
  if (OpenProxmark(&g_session.current_device, serial_port_name, true, 60, true,
                   FLASHMODE_SPEED)) {
  ...
  PrintAndLogEx(SUCCESS,
                "Dump requested from address "_YELLOW_(
                    "%u") ", length "_YELLOW_("%u") "%s.",
...
static int flash_pm3(char *serial_port_name, uint8_t num_files,
                     const char *filenames[FLASH_MAX_FILES], bool can_write_bl,
                     bool force) {
  ...
  PrintAndLogEx(SUCCESS,
                "About to use the following file%s:", num_files > 1 ? "s" : "");
  for (int i = 0; i < num_files; ++i) {
...
int main(int argc, char *argv[]) {
  ...
  // color management:
  // 1. default = no color
  // 2. enable colors if OS seems to support colors and if stdin/stdout aren't
  // redirected
  // 3. load prefs if available, overwrite colors choice if needed
  // 4. disable colors anyway if stdin/stdout are redirected
  //
  // For info, grep --color=auto is doing sth like this, plus test
  // getenv("TERM") != "dumb":
  //   struct stat tmp_stat;
  //   if ((fstat (STDOUT_FILENO, &tmp_stat) == 0) && (S_ISCHR
  //   (tmp_stat.st_mode)) && isatty(STDIN_FILENO))
  ...
  for (int i = 1; i < argc; i++) {
    ...
    if (argv[i][0] != '-') {
      ...
      if (port != NULL) {
        ...
        PrintAndLogEx(
            ERR,
            _RED_("ERROR:") " cannot parse command line. We got " _YELLOW_(
                "%s") " as port and now we got also: " _YELLOW_("%s") "\n",
            port, argv[i]);
    ...
    if (strcmp(argv[i], "-d") == 0 || strcmp(argv[i], "--debug") == 0) {
      if (i + 1 == argc) {
        PrintAndLogEx(
            ERR, _RED_("ERROR:") " missing debugmode specification after -d\n");
        show_help(false, exec_name);
        return 1;
      }
      ...
      if (demod < 0 || demod > 2) {
        PrintAndLogEx(
            ERR, _RED_("ERROR:") " invalid debugmode: -d " _YELLOW_("%s") "\n",
            argv[i + 1]);
        return 1;
      }
      g_debugMode = demod;
    ...
    if (strcmp(argv[i], "-b") == 0 || strcmp(argv[i], "--baud") == 0) {
      ...
      if ((tmpspeed == ULONG_MAX) || (tmpspeed == 0)) {
        PrintAndLogEx(
            ERR, _RED_("ERROR:") " invalid baudrate: -b " _YELLOW_("%s") "\n",
            argv[i + 1]);
        return 1;
      }
      speed = tmpspeed;
    ...
    if (strcmp(argv[i], "-c") == 0 || strcmp(argv[i], "--command") == 0) {
      if (i + 1 == argc) {
        PrintAndLogEx(
            ERR, _RED_("ERROR:") " missing command specification after -c\n");
        show_help(false, exec_name);
        return 1;
      }
      script_cmd = argv[++i];
    ...
    if (strcmp(argv[i], "-y") == 0 || strcmp(argv[i], "--py") == 0) {
      if (i + 1 == argc || strlen(argv[i + 1]) == 0) {
        PrintAndLogEx(
            ERR,
            _RED_(
                "ERROR:") " missing Python script specification after --py\n");
        show_help(false, exec_name);
        return 1;
      }
      script_cmd = argv[++i];
    ...
    if (strcmp(argv[i], "--ncpu") == 0) {
      ...
      if (ncpus < 0 || ncpus >= detected_cpus) {
        PrintAndLogEx(
            ERR,
            _RED_("ERROR:") " invalid number of CPU cores: --ncpu " _YELLOW_(
                "%s") " (available: %d)\n",
  ...
  // Let's take a baudrate ok for real UART, USB-CDC & BT don't use that info
  // anyway
  if (speed == 0)
  ...
  if (script_cmd) {
    ...
    if (strlen(script_cmd) == 0) {
      script_cmd = NULL;
      PrintAndLogEx(ERR, _RED_("ERROR:") " execute command: " _YELLOW_(
                             "command not found") ".\n");
      return 2;
    } else {
      if (addScriptExec) {
  ...
  if (port != NULL) {
    OpenProxmark(&g_session.current_device, port, waitCOMPort, 20, false,
                 speed);
  }

  if (g_session.pm3_present &&
      (TestProxmark(g_session.current_device) != PM3_SUCCESS)) {
    PrintAndLogEx(ERR,
  ...
  if (!g_session.pm3_present) {
    PrintAndLogEx(INFO,
                  _YELLOW_("OFFLINE") " mode. Check " _YELLOW_(
                      "\"%s -h\"") " if it's not what you want.\n",
                  exec_name);
  }

  // ascii art only in interactive client
  if (!script_cmds_file && !script_cmd && g_session.stdinOnTTY &&
      g_session.stdoutOnTTY && !dumpmem_mode && !flash_mode &&
      !reboot_bootloader_mode) {
    showBanner();
  }

  // Save settings if not loaded from settings json file.
  // Doing this here will ensure other checks and updates are saved to over rule
  // default e.g. Linux color use check
  if ((!g_session.preferences_loaded) && (!g_session.incognito)) {
    PrintAndLogEx(
        INFO,
        "Creating initial preferences file"); // json save reports file name, so
                                              // just info msg here
