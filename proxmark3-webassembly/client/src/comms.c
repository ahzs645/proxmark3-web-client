...
// Used by PacketResponseReceived as a ring buffer for messages that are yet to
// be processed by a command handler (WaitForResponse{,Timeout})
...
// Global start time for WaitForResponseTimeout & dl_it, so we can reset timeout
// when we get packets as sending lot of these packets can slow down things
// wuite a lot on slow links (e.g. hw status or lf read at 9600)
static uint64_t timeout_start_time;

static uint64_t last_packet_time;

static bool dl_it(uint8_t *dest, uint32_t bytes, PacketResponseNG *response,
                  size_t ms_timeout, bool show_warning, uint32_t rec_cmd);

// Simple alias to track usages linked to the Bootloader, these commands must
// not be migrated.
// - commands sent to enter bootloader mode as we might have to talk to old
// firmwares
// - commands sent to the bootloader as it only supports OLD frames (which will
// always be the case for old BL)
void SendCommandBL(uint64_t cmd, uint64_t arg0, uint64_t arg1, uint64_t arg2,
                   void *data, size_t len) {
  SendCommandOLD(cmd, arg0, arg1, arg2, data, len);
}

void SendCommandOLD(uint64_t cmd, uint64_t arg0, uint64_t arg1, uint64_t arg2,
  ...
  /**
  This causes hangups at times, when the pm3 unit is unresponsive or
  disconnected. The main console thread is alive, but comm thread just spins
  here. Not good.../holiman
  **/
...
static void SendCommandNG_internal(uint16_t cmd, uint8_t *data, size_t len,
  ...
  if (len > PM3_CMD_DATA_SIZE) {
    PrintAndLogEx(WARNING, "Sending %zu bytes of payload is too much, abort",
                  len);
    return;
  }
  ...
  **/
  ...
  if ((g_conn.send_via_fpc_usart && g_conn.send_with_crc_on_fpc) ||
      ((!g_conn.send_via_fpc_usart) && g_conn.send_with_crc_on_usb)) {
...
#ifdef COMMS_DEBUG_RAW
  print_hex_break((uint8_t *)&txBufferNG.pre, sizeof(PacketCommandNGPreamble),
                  32);
  if (ng) {
    print_hex_break((uint8_t *)&txBufferNG.data, len, 32);
  } else {
    print_hex_break((uint8_t *)&txBufferNG.data, 3 * sizeof(uint64_t), 32);
    print_hex_break((uint8_t *)&txBufferNG.data + 3 * sizeof(uint64_t),
                    len - 3 * sizeof(uint64_t), 32);
  }
...
void SendCommandMIX(uint64_t cmd, uint64_t arg0, uint64_t arg1, uint64_t arg2,
                    const void *data, size_t len) {
...
/**
 * @brief This method should be called when sending a new command to the pm3. In
 * case any old responses from previous commands are stored in the buffer, a
 * call to this method should clear them. A better method could have been to
 * have explicit command-ACKS, so we can know which ACK goes to which operation.
 * Right now we'll just have to live with this.
 */
...
static void storeReply(const PacketResponseNG *packet) {
  ...
  if ((cmd_head + 1) % CMD_BUFFER_SIZE == cmd_tail) {
    ...
    //  circular buffer.
    PrintAndLogEx(FAILED, "WARNING: Command buffer about to overwrite command! "
...
static void PacketResponseReceived(PacketResponseNG *packet) {
  ...
  (void)prev_clk;
  //    PrintAndLogEx(NORMAL, "[%07"PRIu64"] RECV %s magic %08x length %04x
  //    status %04x crc %04x cmd %04x",
  //                clk - prev_clk, packet->ng ? "NG" : "OLD", packet->magic,
  //                packet->length, packet->status, packet->crc, packet->cmd);
  ...
  case CMD_DEBUG_PRINT_STRING: {
    ...
    if (flag & FLAG_LOG) {
      ...
      // PrintAndLogEx(NORMAL, "[" _MAGENTA_("pm3") "] ["_BLUE_("#")"] " "%s",
      // s);
...
void *uart_reconnect(void *targ) {
  ...
  while (1) {
    // throttle
    msleep(200);
    if (OpenProxmarkSilent(&g_session.current_device,
                           connection->serial_port_name, speed) == false) {
      continue;
    }
...
static void
  ...
  while (connection->run) {
    ...
    if (is_receiving_raw) {
      uint8_t *bufferData =
          __atomic_load_n(&comm_raw_data, __ATOMIC_SEQ_CST); // read only
      size_t bufferLen =
          __atomic_load_n(&comm_raw_len, __ATOMIC_SEQ_CST); // read only
      size_t bufferPos =
          __atomic_load_n(&comm_raw_pos, __ATOMIC_SEQ_CST); // read and write
      if (bufferPos < bufferLen) {
        ...
        if (res == PM3_SUCCESS) {
          ...
          PrintAndLogEx(WARNING, "Error when reading raw data: %zu/%zu, %d",
                        bufferPos, bufferLen, res);
          error = true;
          if (res == PM3_ENOTTY) {
            commfailed = true;
          }
        ...
        // Ignore data when bufferPos >= bufferLen and is_receiving_raw has not
        // been set to false
      ...
      res = uart_receive(sp, (uint8_t *)&rx_raw.pre,
                         sizeof(PacketResponseNGPreamble), &rxlen);
      ...
      if ((res == PM3_SUCCESS) && (rxlen == sizeof(PacketResponseNGPreamble))) {
        ...
        if (rx.magic == RESPONSENG_PREAMBLE_MAGIC) { // New style NG reply
          ...
          if (length > PM3_CMD_DATA_SIZE) {
            PrintAndLogEx(
                WARNING,
                "Received packet frame with incompatible length: 0x%04x",
                length);
            error = true;
          }
          ...
          if ((!error) && (length > 0)) { // Get the variable length payload
            ...
            if ((res != PM3_SUCCESS) || (rxlen != length)) {
              ...
              PrintAndLogEx(
                  WARNING,
                  "Received packet frame with variable part too short? %d/%d",
                  rxlen, length);
              error = true;
              ...
              if (rx.ng) { // Received a valid NG frame
                ...
                rx.length = length;
                if ((rx.cmd == g_conn.last_command) &&
                    (rx.status == PM3_SUCCESS)) {
                  ACK_received = true;
                }
                ...
                if (length < sizeof(arg)) {
                  PrintAndLogEx(WARNING,
                                "Received MIX packet frame with incompatible "
                                "length: 0x%04x",
                                length);
                  error = true;
                }
          ...
          if (!error) { // Get the postamble
            res = uart_receive(sp, (uint8_t *)&rx_raw.foopost,
                               sizeof(PacketResponseNGPostamble), &rxlen);
            if ((res != PM3_SUCCESS) ||
                (rxlen != sizeof(PacketResponseNGPostamble))) {
          ...
          if (!error) { // Check CRC, accept MAGIC as placeholder
            ...
            if (rx.crc != RESPONSENG_POSTAMBLE_MAGIC) {
              ...
              if ((first << 8) + second != rx.crc) {
                PrintAndLogEx(
                    WARNING,
                    "Received packet frame with invalid CRC %02X%02X <> %04X",
                    first, second, rx.crc);
                error = true;
              }
            }
...
#endif
#ifdef COMMS_DEBUG_RAW
            print_hex_break((uint8_t *)&rx_raw.pre,
                            sizeof(PacketResponseNGPreamble), 32);
            print_hex_break((uint8_t *)&rx_raw.data, rx_raw.pre.length, 32);
            print_hex_break((uint8_t *)&rx_raw.foopost,
                            sizeof(PacketResponseNGPostamble), 32);
#endif
          ...
          res = uart_receive(
              sp, ((uint8_t *)&rx_old) + sizeof(PacketResponseNGPreamble),
          ...
          if ((res != PM3_SUCCESS) ||
            ...
            PrintAndLogEx(
                WARNING,
                "Received packet OLD frame with payload too short? %d/%zu",
                rxlen,
                sizeof(PacketResponseOLD) - sizeof(PacketResponseNGPreamble));
            error = true;
          }
        ...
        if (rxlen > 0) {
          PrintAndLogEx(WARNING,
                        "Received packet frame preamble too short: %d/%zu",
    ...
    if (connection->block_after_ACK) {
      ...
      // transmitted This is only working on OLD frames, and only used by
      // flasher and flashmem
      if (ACK_received) {
#ifdef COMMS_DEBUG
        PrintAndLogEx(NORMAL,
...
bool OpenProxmarkSilent(pm3_device_t **dev, const char *port, uint32_t speed) {
  ...
  if (sp == INVALID_SERIAL_PORT) {
    ...
    // "Session" flag, to tell via which interface next msgs should be sent: USB
    // or FPC USART
...
bool OpenProxmark(pm3_device_t **dev, const char *port, bool wait_for_port,
                  int timeout, bool flash_mode, uint32_t speed) {

  if (wait_for_port == false) {
    PrintAndLogEx(SUCCESS, "Using UART port " _GREEN_("%s"), port);
    sp = uart_open(port, speed, false);
  } else {
    PrintAndLogEx(SUCCESS, "Waiting for Proxmark3 to appear on " _YELLOW_("%s"),
                  port);
    fflush(stdout);
    int openCount = 0;
    ...
    } while (++openCount < timeout &&
             (sp == INVALID_SERIAL_PORT || sp == CLAIMED_SERIAL_PORT));
  }

  // check result of uart opening
  if (sp == INVALID_SERIAL_PORT) {
    PrintAndLogEx(WARNING,
                  "\n" _RED_("ERROR:") " invalid serial port " _YELLOW_("%s"),
                  port);
    PrintAndLogEx(
        HINT, "Try the shell script " _YELLOW_(
                  "`./pm3 --list`") " to get a list of possible serial ports");
    sp = NULL;
    return false;
  } else if (sp == CLAIMED_SERIAL_PORT) {
    PrintAndLogEx(WARNING,
                  "\n" _RED_("ERROR:") " serial port " _YELLOW_(
                      "%s") " is claimed by another process",
                  port);
    PrintAndLogEx(
        HINT, "Try the shell script " _YELLOW_(
                  "`./pm3 --list`") " to get a list of possible serial ports");
...
int TestProxmark(pm3_device_t *dev) {
  ...
  if ((resp.length != sizeof(g_pm3_capabilities)) ||
    ...
    PrintAndLogEx(ERR, _RED_("Capabilities structure version sent by Proxmark3 "
                             "is not the same as the one used by the client!"));
    PrintAndLogEx(
        ERR,
        _RED_(
  ...
  bool is_tcp_conn =
      (g_conn.send_via_ip == PM3_TCPv4 || g_conn.send_via_ip == PM3_TCPv6);
  bool is_bt_conn = (memcmp(g_conn.serial_port_name, "bt:", 3) == 0);
  bool is_udp_conn =
      (g_conn.send_via_ip == PM3_UDPv4 || g_conn.send_via_ip == PM3_UDPv6);

  PrintAndLogEx(SUCCESS, "Communicating with PM3 over %s%s%s%s",
                (g_conn.send_via_fpc_usart) ? _GREEN_("FPC UART")
                                            : _GREEN_("USB-CDC"),
  ...
  if (g_conn.send_via_fpc_usart) {
    ...
    if (g_conn.send_via_local_ip) {
      // (g_conn.send_via_local_ip == true) -> ((is_tcp_conn || is_udp_conn) ==
      // true)
      res = uart_reconfigure_timeouts(
          is_tcp_conn ? UART_TCP_LOCAL_CLIENT_RX_TIMEOUT_MS
...
// Max communication delay is when sending largest frame and receiving largest
// frame Empirical measures on FTDI with physical cable: "hw pingng 512"
//    usb ->    6..32ms
// 460800 ->   40..70ms
//   9600 -> 1100..1150ms
 ...
 */
size_t WaitForRawDataTimeout(uint8_t *buffer, size_t len, size_t ms_timeout,
                             bool show_process) {
  ...
  while (pos < len) {
    ...
    if (last_pos == pos) {
      ...
      // If ms_timeout == -1, the loop can only be breaked by pressing Enter or
  ...
  if (pos == len && (ms_timeout != (size_t)-1)) {
    ...
    // to stop the current process, and wait for some time to make sure the
    // process has been stopped. If ms_timeout == -1, the user might not want to
    // break the existing process on the arm side.
    SendCommandNG(CMD_BREAK_LOOP, NULL, 0);
    msleep(ms_timeout);
  }
 ...
 */
bool WaitForResponseTimeoutW(uint32_t cmd, PacketResponseNG *response,
                             size_t ms_timeout, bool show_warning) {
...
bool WaitForResponseTimeout(uint32_t cmd, PacketResponseNG *response,
                            size_t ms_timeout) {
 ...
 * @return true if command was returned, otherwise false
 */
bool GetFromDevice(DeviceMemType_t memtype, uint8_t *dest, uint32_t bytes,
                   uint32_t start_index, uint8_t *data, uint32_t datalen,
                   PacketResponseNG *response, size_t ms_timeout,
                   bool show_warning) {
  ...
  case BIG_BUF: {
    ...
    return dl_it(dest, bytes, response, ms_timeout, show_warning,
  ...
  while (true) {
    ...
    if (getReply(response)) {
      ...
      if (response->cmd == CMD_SPIFFS_DOWNLOAD &&
          response->status == PM3_EMALLOC)
        return false;
      // Spiffs // fpgamem-plot download is converted to NG,
      if (response->cmd == CMD_SPIFFS_DOWNLOAD ||
          response->cmd == CMD_FPGAMEM_DOWNLOAD)
        return true;
      ...
      if (response->cmd == rec_cmd) {
        ...
        if (offset + copy_bytes > bytes) {
          PrintAndLogEx(FAILED,
                        "ERROR: Out of bounds when downloading from device,  "
                        "offset %u | len %u | total len %u > buf_size %u",
                        offset, copy_bytes, offset + copy_bytes, bytes);
          break;
        }

        memcpy(dest + offset, response->data.asBytes, copy_bytes);
        bytes_completed += copy_bytes;
      } else if (response->cmd == CMD_WTX &&
